"""ROUND 18.2 — Orphan Migration Dry-Run (NO WRITE).

Analizza il piano di migrazione per le 5 classi live orphan verso i target
canonical R18 sigillati dal PM (Q1-Q5). Produce solo find + report, ZERO
update_many, ZERO effetto sui doc.

Mapping sigillato:
  priest      → paladin              (Q1, 190 adv previsti)
  ranger      → cacciatore_mostri    (Q2, 175 adv previsti)
  warlock     → cacciatore_vuoto     (Q3, 128 adv previsti)
  berserker   → warrior              (Q4, 3 adv previsti — alias/deprecated)
  assassin    → rogue                (Q5, 0 adv previsti — zero migration)

Genera:
  - `/app/memory/round182_orphan_migration_dry_run.json` (raw JSON)

Uso:
    python -m app.scripts.round182_orphan_migration_dry_run
"""
...
MIGRATION_MAP: dict[str, dict] = {
    ...
    "ranger": {
        "target": "cacciatore_mostri",
        "target_canonical_it": "Cacciatore di Mostri",
        "target_exists_live": False,
        "type": "migration_to_new_class",
        "pm_decision": "Q2",
        "expected_adv": 175,
    },
    "warlock": {
        "target": "cacciatore_vuoto",
...
async def analyze_source(db, slug: str, plan: dict) -> dict:
    ...
    # Talent progress (should be 0 in R18.2 PILOT)
    ...
    # Item pool coverage for target class
    ...
    # Also check catalog class existence
    target_class_doc = await db.adventurer_classes.find_one(
        {"slug": plan["target"]}, {"_id": 0}
    )

    # Sample of items currently equipped by source-class adventurers
    # (check if they'd be off-class after migration)
    ...
    async for adv in db.adventurers.find(query, {"_id": 0, "id": 1, "name": 1,
        ...
        for item_id in item_ids[:2]:
            if not item_id:
                continue
            it = await db.items.find_one({"id": item_id}, {"_id": 0, "recommended_classes": 1, "name": 1})
            if it:
...
async def run() -> int:
    ...
    print(f"[mode] DRY-RUN · db={db_name} · round=R18.2 orphan-migration-plan")
    ...
    for source_slug, plan in MIGRATION_MAP.items():
        ...
        print(f"  actual_adv={r['actual_adv_count']}  guilds={r['guild_ids_impacted_count']}  "
              f"in_expedition={r['n_in_expedition']}  target_item_pool={r['target_item_pool_size']} "
    ...
    output = {
        ...
        "round": "R18.2",
        "phase": "PILOT_dry_run",
        "sealed_pm_decisions": ["Q1", "Q2", "Q3", "Q4", "Q5"],
        ...
        "rollback_snapshot_recipe": {
            "step_1_snapshot": (
                "BEFORE apply: dump adventurers.class_slug into career_history "
                "as {event_type: 'r18_migration', class_slug_before, "
                "class_slug_after, migrated_at}"
            ),
            "step_2_apply": "update_many({class_slug: source}, {$set: {class_slug: target, r18_migrated_at: iso, r18_migrated_from: source}})",
            "step_3_rollback": (
                "IF rollback needed: update_many({r18_migrated_from: source}, "
                "{$set: {class_slug: '$r18_migrated_from'}, $unset: "
                "{r18_migrated_at: 1, r18_migrated_from: 1}})"
            ),
        },
    }

    out_path = Path("/app/memory/round182_orphan_migration_dry_run.json")
