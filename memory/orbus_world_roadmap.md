...
| **R18.3a** | ✅ CLOSED (2026-07-04T19:55Z) | Class Migration Pre-Req — seed classi `cacciatore_di_mostri` + `cacciatore_del_vuoto` in `adventurer_classes` (is_playable=false, migration_target_only=true, source_round=R18.3a). Bridge item append-only: 31 ranger items + 18 warlock items via `$addToSet`. Dry-run migration script aggiornato con slug corretti + slug_correction_note. Audit event `R18_CLASS_MIGRATION_PREREQ_READY` (idempotente). Test: 16/16 PASS (regression 61/61 con R18.1+R18.2+R18.1.2). Report: `round183a_class_migration_prereq_report.md`. |
