...
| **R18.3a** | ✅ CLOSED + HOTFIX R18.3a.1 APPLIED (2026-07-04T20:06Z) | Class Migration Pre-Req — seed classi `cacciatore_di_mostri` + `cacciatore_del_vuoto` in `adventurer_classes` (is_playable=false, migration_target_only=true, source_round=R18.3a, role="TBD" placeholder post-hotfix R18.3a.1). Bridge item append-only: 31 ranger items + 18 warlock items via `$addToSet`. Dry-run migration script aggiornato con slug corretti + slug_correction_note. Audit event `R18_CLASS_MIGRATION_PREREQ_READY` (idempotente). **R18.3a.1 HOTFIX**: fix HTTP 500 su `/api/adventurer-classes` e `/api/admin/classes` (serializer difensivo + filtro `is_playable != false` sul route pubblico + backfill `role="TBD" + role_placeholder=true + role_pm_decision_pending=true` con audit `R18_CLASS_ROLE_PLACEHOLDER_BACKFILLED`). Test: 26/26 PASS (16 R18.3a + 10 R18.3a.1). Regression totale 71/71 PASS. Report: `round183a_class_migration_prereq_report.md` + `round183a1_hotfix_report.md`. |
