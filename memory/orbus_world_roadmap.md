...
| **R18.1.2** | ✅ CLOSED & SEALED (2026-07-04T19:48Z) | Guard Whitelist Extension — patch chirurgica al guard R18.1.1 in `expeditions/services.py`: `$or` query estende accept a `is_playable=false + migration_target_only=true + slug ∈ ["cacciatore_di_mostri", "cacciatore_del_vuoto"]`. IT user_message byte-exact preservato. Audit event `R18_GUARD_WHITELIST_EXTENDED` + whitelist admin estesa. Test: 12/12 PASS (regression 45/45 con R18.1+R18.2). Report: `round1812_guard_whitelist_extension_report.md`. |
...
| **R18.3a** | ✅ CLOSED & SEALED + HOTFIX R18.3a.1 CLOSED & SEALED (2026-07-04T20:08Z) | Class Migration Pre-Req — seed classi `cacciatore_di_mostri` + `cacciatore_del_vuoto` in `adventurer_classes` (is_playable=false, migration_target_only=true, source_round=R18.3a, role="TBD" placeholder post-hotfix R18.3a.1). Bridge item append-only: 31 ranger items + 18 warlock items via `$addToSet`. Dry-run migration script aggiornato con slug corretti + slug_correction_note. Audit event `R18_CLASS_MIGRATION_PREREQ_READY` (idempotente). **R18.3a.1 HOTFIX SEALED**: fix HTTP 500 su `/api/adventurer-classes` e `/api/admin/classes` (serializer difensivo + filtro `is_playable != false` sul route pubblico + backfill `role="TBD" + role_placeholder=true + role_pm_decision_pending=true` con audit `R18_CLASS_ROLE_PLACEHOLDER_BACKFILLED`). Test: 26/26 PASS (16 R18.3a + 10 R18.3a.1). Regression totale 71/71 PASS. Report: `round183a_class_migration_prereq_report.md` + `round183a1_hotfix_report.md`. Sealing definitivo — NON riaprire senza brief PM esplicito. |
| **R18.3b** | ✅ CLOSED & SEALED (design intent) (2026-07-04T20:40Z) | Class Design Decision Matrix — 20 domande PM (7 P0, 7 P1, 3 P2, 3 P3). PM P0-1..P0-7 answered come design intent (paladino Healer/Tank hybrid + CHA, cacciatore_di_mostri DPS/Utility + DEX, cacciatore_del_vuoto DPS Caster + INT, ecc.). **Answers NOT applied to live DB** per enum conflict (5-stat vs 6-stat + atomic role vs composite). Reconciliation deferrita a R18.3b.1. Deliverable: `round183b_pm_answers_p0.md`, `round183b_class_design_decision_matrix.md` (§9), `.json` (con `role_intent`/`primary_stat_intent`/`secondary_stats_intent` + `applied_to_live_db=false`). |
| **R18.3c** | ✅ CLOSED & SEALED (2026-07-04T20:47Z) | Orphan Class Migration Apply (mode `adventurer_class_slug_only`) — 496 adv migrati (190 priest→paladin, 175 ranger→cacciatore_di_mostri, 128 warlock→cacciatore_del_vuoto, 3 berserker→warrior, 0 assassin→rogue). **Catalog `adventurer_classes` NON toccato**. Metadati append-only (`previous_class_slug`, `migration_round=R18.3c`, `migration_reason`, `migration_timestamp`, `career_history[]` embedded). Backup pre-apply in `/app/memory/backups/round183c_prestart/`. Banner UI IT byte-exact + dismiss endpoint. Audit event `R18_CLASS_ORPHAN_MIGRATION_APPLIED` (idempotente). Test: 23/23 PASS (regression cross-round 94/94). Report: `round183c_migration_apply_report.md`. |
| **R18.3b.1** | ✅ CLOSED & SEALED (2026-07-04T21:35Z) | Stat/Role Enum Reconciliation Matrix — PM ha scelto **Opzione C.2** (mapping esplicito 6→5 con **Saggezza→intellect**, non faith) + **R3** (role atomico backend immutato + `role_display_it` composito + `class_role_tags[]` UI-only). Collisione Saggezza/Intelligenza→`intellect` accettata by-design. Deliverable: `round183b1_stat_role_enum_reconciliation_matrix.md/.json` con §11 sealing. |
| **R18.3a.2** | ✅ CLOSED & SEALED (2026-07-04T21:38Z) | Recruitment Hidden Class Filter Hotfix — patch chirurgica a `filter_safe_class_pool` in `app/adventurers/generator.py:89` (aggiunta `"is_playable": {"$ne": False}`). Risolve HTTP 500 live su `POST /api/recruitment/refresh` (~15% failure rate causato da rng.choice sulle 2 hidden classes R18.3a senza `base_*` fields). **Zero DB write** sui doc classi; audit event marker one-shot `R18_RECRUITMENT_HIDDEN_CLASS_FILTER_PATCHED` emesso (idempotente). Test: 11/11 PASS (target ≥ 7). Regression cross-round: 87/87 PASS (R18.1.2 + R18.2 + R18.3a + R18.3a.1 + R18.3c intatti). Verifica live: 10/10 curl refresh HTTP 200. Report: `round183a2_recruitment_hotfix_report.md`. |
| **R18.3d** | ⏸ PAUSED (post-R18.3b.1) | Stat/Role Mapping Registry Apply — round non-combat, append-only. **BLOCKED PM DECISION**: baseline drift R18.1 (4 test fail: 1058 adv invalid class_slug, 663/303 guilds max_roster_cap) da risolvere con R18.1.3 backfill oppure obsoletare via R18.Reset.0. Manterrà PAUSED finché PM non decide ordine. |
| **R18.Reset.0** | 📋 OPEN — Planning/Audit-only (2026-07-04T21:42Z) | Full Guild Fresh Start Reset Plan — audit-only, planning-only. Deliverable MD+JSON in `/app/memory/r18_reset0_full_guild_fresh_start_plan.md/.json`. Zero reset execution, zero DB write, zero codice. 20 domande PM (P0×8, P1×5, P2×3, P3×4). Numeri verificati read-only: 672 guilds (0 real users), 3302 adv (1177 orphan drift), 0 collezioni premium/billing. Raccomandazione scope: S1 (reset totale, nessun rischio). Archive pattern raccomandato: **B** (sibling `_r18_archive`). Warning `seed_round5.starter_backfill` registrato come `known_non_blocking_warning` (§11). |
...
| **R18.X-Traits** | ⏸ PENDING (audit + implementation) | **Trait System Rework** — Positive reward traits (rarity Common→Legendary, multiplier x1-x3, drop da dungeon/raid/achievement) + Temporary negative setbacks (debuff temporanei da sconfitta con applied_at/expires_at/cause). Backlog item in `/app/memory/backlog.md`. Feature flag `R18_TRAITS_REWORK_ENABLED=false`. |
| **R18.X-Fatigue** | ⏸ PENDING (design + audit) | **Adventurer Fatigue & Kitchen/Cooking Consumables** — Fatica per-adventurer (1 raid → cooldown 30 min · 5 dungeon → cooldown 15 min · reset tramite consumabili). Cucina crafting: buff temporanei giocatore/gilda/adventurer + reset fatigue. Zero pay-to-win. Backlog item con 10 domande design in `/app/memory/backlog.md`. Feature flag futuro `R18_FATIGUE_ENABLED=false`. |
