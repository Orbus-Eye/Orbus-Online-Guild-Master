...
| **R18.3b** | 🔍 OPEN (decision support, audit-only) | Class Design Decision Matrix — round puramente decisionale, ZERO implementazione. Prepara materiale ragionato per far decidere PM le domande Q7-Q24 su ruoli, stat, armor, sovrapposizioni delle 27 classi canoniche. Nessun DB write. Nessuna migration. Nessun seed. Nessun talento reale. Nessun item bridge nuovo. Solo lettura dai file sorgente in `/app/memory/source_materials/r18_27_class_sources/` + query READ contestuali. Deliverable: `round183b_class_design_decision_matrix.md` + `.json`. |
| **R18.3** | ⏸ DEFERRED (bloccato da R18.3b decisioni PM P0) | Apply reale migration 496 orphan → target classes. Pre-req R18.3a DONE ma decisione PM su role/stat/armor delle 5 migration-critical classes (paladin/warrior/rogue/cacciatore_di_mostri/cacciatore_del_vuoto) NON risolta. NON eseguire senza brief PM esplicito post-R18.3b. |
