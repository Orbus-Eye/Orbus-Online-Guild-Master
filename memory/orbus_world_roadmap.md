...
| **R18.3d** | 🚀 OPEN (post-R18.3b.1) | Stat/Role Mapping Registry Apply — round non-combat, append-only. Crea registry autorevole in `/app/memory/r18_stat_role_mapping_registry.md/.json`. Applica catalog metadata append-only ai 13 doc `adventurer_classes` live: `design_primary_stat_it`, `design_secondary_stats_it`, `mapped_primary_stat_live`, `mapped_secondary_stats_live`, `role_display_it`, `class_role_tags[]`, `role_mapping_source_round="R18.3d"`. Zero schema migration, zero combat math change, zero adventurers change. Audit event `R18_STAT_ROLE_MAPPING_REGISTERED`. |
