...
| **R18.Reset.0** | 📋 PLANNING PENDING | Full Guild Fresh Start Reset Plan (audit-only, planning-only) — deliverable `r18_reset0_full_guild_fresh_start_plan.md/.json`. Zero reset execution. Zero DB write. Preserve: guild name/owner/user/account/legal/premium. Reset candidates: gold, roster, items, dungeons, raids, leaderboards, achievements. Snapshot pre-reset obbligatorio. Rollback plan. Domande PM finali ≥ 10. |
